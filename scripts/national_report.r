...
date_of_highest_average <- filter(first, average_initial_claims == highest_average) %>% 
  select(Filed_week_ended)


# Claims from January 5, 2019 to May 9, 2020 (Including Pandemic)
ggplot(data = first, aes(x=Filed_week_ended, y = average_initial_claims, group=1)) +
   geom_line()+
   geom_point()+
   scale_x_date(breaks = first$Filed_week_ended) +  #code found https://stackoverflow.com/questions/51604367/show-all-date-values-on-ggplot-x-axis-r
                                                    # shows all dates in the x axis

  ylim(0, max(first$average_initial_claims) + 500) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + # made the data label vertical (hence the 90 degree angle)
...
# Pre-pandemic: huge increase removed (>15000 claims)
ggplot(data = first, aes(x=Filed_week_ended, y = average_initial_claims, group=1)) +
  geom_line()+
  geom_point()+
  scale_x_date(breaks = first$Filed_week_ended) +
  
  #set limit here to not include the quick high peak due to pandemic
  ylim(0, 15000) +
