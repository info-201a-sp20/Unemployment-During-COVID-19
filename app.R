...
ui <- tagList(navbarPage(
  ...
  tabsetPanel(id = "tabs",
    ...
    fear and uncertainty regarding the state of national public health, as well
    as concerns for the future of the economy. Around the globe, many countries
    have taken a hard hit to their economies across all industries and many have
    been faced with unemployment. The United States is no stranger to this
    issue, as delayed responses to the initial spread of the virus played a
    large role in its rapid spread, further accelerating the epidemic and the
    resulting economic downturn."), br(),
    p("This project looks at the effects of COVID-19 on unemployment in
    Washington and the United States as a whole. We will refer to datasets that
    show the spread of the virus, as well as national and local unemployment
    claims for Washington state. This is important because the quarantine has
    had a large impact on the local and national economy, and looking at this
    information can help us be prepared for unemployment trends in future waves
    of the virus."), br(),
    h3("Data Sources"), br(), 
    h4(a("COVID-19 in USA", 
         href = "https://www.kaggle.com/sudalairajkumar/covid19-in-usa/?select=us_states_covid19_daily.csv"), 
       "by Sudalai Raj Kumar"),
    br(), 
    ...
    tabPanel("Conclusion", value = 4,
      ...
      Predict"), br(),
      p("As the data showcase not only the total cases confirmed, we could also
      tell how our healthcare resources, e.g. hospital, ICU, are used
      nationally. We could also track the recovered number to tell how each
      state is reacting with the pandemic. Through the area plot, we could tell the peak of increasing
                cases was in April, with 4 days of more than 10k of positive
                increase cases. The rest of the month still remained a high
                number of increasing positive cases in between 5k to 10k. Due to
                the high number of positive increase in single day, the number
                of currently hospitalized became high after cases confirmed. In
                April, there were 6 days with more than 15k people
                hospitalized."), br(),
                p("According to the data provided, we could predict that as the
                positive cases grow, the growth of hospitalized cases will come
                after. To prevent the healthcare system overload, it is
                important to flatten the curve of the confirmed cases, so the
                hospitals are able to distribute the more resources to each
                patient. When the number of positive increase cases went down,
                hospitals may have more time to response and purchase more
                supplies. Another major effect of short in healthcare resources
                was the rise in death rate. As hospitals are not able to provide
                enough healthcare until the patient recover, the time for
                patients to recover by themselves become shorter thus causes
                unnecessary deaths. As the data is updated by daily basis, it
                could help the government to track the cases and check which
                state has been affected the most so far. The public sectors are
                then able determine which state they should fund more for the
                healthcare resources. Through the number of cases in ICU and on
                ventilators, governments are also able to tell which healthcare
                they should purchase/produce more to support the increasing
                number of hospitalized. In conclusion, the insights help the
                government to puzzle out which healthcare resources they are
                lack in and prepare for similar situation in the future."), 
                br(),
                ...
                few weeks into May."), br(),
                h4("What We Learned from the Data and What We Can Use Our Data 
                To Predict"), br(),
                p("Initially we wanted to look at what the unemployment looked
                like on average across the US. While we didn’t brainstorm on a
                ...
                March 2020."), br(),
                p("Based on the data provided, we can assume that this is when
                the lockdown measures were taken place, causing many to lose
                jobs and in result, file unemployment cases. There is a
                significant jump in the beginning of March. Alabama for example,
                shows the claims spike from March 14th (4,752 claims), br(),
                exponentially increases for the next few dates (55,335 to 
                113,641 claims). This data helps us see the trends of specific
                ...
                at what states are affected and how badly they are affected.
                By analyzing the data, organizations can predict the amount of
                ...
                offer for those that are affected by the pandemic."), br(),
                ...
                to either level off or trend downwards."), br(),
                h4("What We Learned from the Data and What We Can Use Our Data
                To Predict"), br(),
                ...
                around 48000 unemployment claims."), br(),
                p("From the data above, we have been able to see trends
                throughout the weeks of the pandemic in the state of Washington.
                As the data and graph is continuously updated, users will be
                able to see if and when a more consistent trend will arise, and
  ...
  helpText("This line chart displays the trend line of the average unemployment
  claim rates within the US. All 50 states are included and can be viewed via
  selecting the state from the select box.")),
